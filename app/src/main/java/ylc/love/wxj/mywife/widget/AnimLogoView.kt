...
class AnimLogoView @JvmOverloads constructor(
    ...
    private fun initPaint() {
        ...
        mPaint!!.typeface = FontCustom.setFont(context)
    }
    ...
    private fun initOffsetAnimation() {
        if (mOffsetAnimator == null) {
            ...
            mOffsetAnimator?.interpolator = AccelerateDecelerateInterpolator()
    ...
    private fun initLogoCoordinate() {
        ...
        for (i in 0 until mLogoTexts.size()) {
            mRadonPoints.put(
                i, PointF(
                    Math.random().toFloat() * mWidth, Math.random().toFloat() * mHeight
                )
            )
        }
    }
    ...
    companion object {
        private const val DEFAULT_LOGO = "yang lin cai"
